    ...
    state.logs.forEach((log, index) => {
        ...
        if (!existing) {
            ...
            item.innerHTML = `
                ...
                <button class="desktop-delete-btn" onclick="deleteAction('${log.id}')">DELETE</button>
            `;
...
function setupControls() {
    ...
    logList.addEventListener('pointerdown', e => {
        // Prevent action on the delete button itself
        if (e.target.closest('.desktop-delete-btn')) return;
        ...
        // Only capture pointer to avoid selecting text when swiping
        activeItem.setPointerCapture(e.pointerId);
    });

    logList.addEventListener('pointermove', e => {
    ...
    logList.addEventListener('pointerup', resetActiveItem);
    logList.addEventListener('pointercancel', resetActiveItem);
}
...
function setupSystemModal() {
    ...
    if (!systemBtn || !systemModal) return;
    ...
    exportBtn.addEventListener('click', () => {
        ...
        dlAnchorElem.setAttribute("download", `zev_state_${getTodayString()}.json`);
        dlAnchorElem.click();
    });
    ...
    wipeBtn.addEventListener('click', () => {
        if (confirm("WARNING: This will permanently format and erase all ZEV system data across all cycles. Proceed?")) {
            localStorage.removeItem(STORAGE_KEY);
            state = { ...defaultState, logs: [], history: {} }; // Reset state completely
            saveState();
            renderUI();
            systemModal.classList.add('hidden');
        }
